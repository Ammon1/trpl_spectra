...
plt.plot(np.log(frame_g.iloc[2,:].values))
plt.plot(np.log(frame_g.iloc[3,:].values))
plt.plot(np.log(frame_g.iloc[4,:].values))
...
plt.plot(X[p1:p2],np.log(spectra.iloc[p1:p2,40:300].T.sum().iloc[:1000].values))

spectra_g=frame_g.apply(lambda y: make_spectrum(y,w_g))
X_g=np.arange(0,15797.76,(15797.76/(spectra_g.shape[0]))).astype(int)

plt.plot(X_g[p1:p2],np.log(spectra_g.iloc[p1:p2,1].values))

plt.plot(frame.iloc[1000:1200,40].values)
plt.plot(rfft(frame.iloc[1000:1200,40].values))
